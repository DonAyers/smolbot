...
class ImprovementManager {
    constructor() {
        ...
        this.configPath = path.join(projectRoot, 'improvement-config.json');
    ...
    createTask(name, description, targetComponent) {
        ...
        // Create task folder
        const taskFolder = path.join(this.projectRoot, this.config.config.tasks.folder, task.id);
    ...
    checkInspiration() {
        const inspoFolder = path.join(this.projectRoot, this.config.config.inspiration.folder);
    ...
    async captureScreenshots(taskId, count = 5) {
        ...
        const taskFolder = path.join(this.projectRoot, this.config.config.tasks.folder, taskId);
    ...
    async compareIteration(taskId, iteration) {
        ...
        for (const file of beforeFiles) {
            ...
            console.log(`   └─ Diff saved: ${path.relative(this.projectRoot, diffPath)}\n`);
        ...
        console.log(`   Metrics saved: ${path.relative(this.projectRoot, path.join(iterFolder, 'visual-metrics.json'))}\n`);
