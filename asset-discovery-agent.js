...
Output should be suitable for Phaser.js game engine.`;
    ...
    createAssetRequest(taskId, assetType, description, styleReferences) {
        ...
        const requestFile = path.join(__dirname, 'improvement-tasks', taskId, `asset-request-${timestamp}.md`);
...
2. **DOCUMENT FINDINGS**
   ...
   Create: ${path.join(__dirname, 'improvement-tasks', taskId, 'asset-findings.md')}
...
`);
    ...
    const promptFile = path.join(__dirname, 'improvement-tasks', taskId, 'asset-discovery-prompt.txt');
